...
class CheckIfRegisterFieldsAreAvailableMonkeyTest {
    @Test
    fun runMonkeyTest() {
        ...
        val timeout = 500L
        val registerButton = device.wait(Until.findObject(By.res("com.example.mobilnyciuch:id/btn_register")), timeout)
        ...
        val confirmPasswordField = device.wait(Until.findObject(By.res("com.example.mobilnyciuch:id/confirmPasswordEditText")), timeout)

        // Check if register screen elements are found
        assertTrue("Register screen elements not found", usernameField != null && passwordField != null && confirmPasswordField != null)
        scenario.close()
    }
}
